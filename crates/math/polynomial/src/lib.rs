...
/// ### Groups
///
/// 群とは、空でない集合 $G$ と二項演算 $\\op\\colon G\\times G\\to G$
/// であって、次の条件を満たすものを言う。
///
/// - 結合律 (*associativity*) を満たす。$a, b, c, \\in G \\implies (a\\op b)\\op c = a\\op(b\\op c)$。
/// - 単位元 (*identity*) を持つ。ある $1\\in G$ に対して、$a\\in G \\implies a\\op 1 = 1\\op a = a$。
/// - 逆元 (*inverse*) を持つ。任意の $a\\in G$ に対して $a^{-1} \\in G$ が存在して $a\\op a^{-1} = a^{-1}\\op a = 1$。
///
/// この群は $(G; \\op, 1, {}^{-1})$ と書かれる。$a\\op b$ を単に $ab$ とも書く。
///
/// 二つの演算を使い分けたいとき、$\\times$ の代わりに $+$ を、$1$ の代わりに $0$
/// を、$a^{-1}$ の代わりに $-a$ と書くこともある。
/// 前者の群を乗法群 (*multiplicative group*) と呼ぶ。後者の群は $(G; +, 0, -1)$
/// と書き、加法群 (*additive group*) と呼ぶ。$a + (-b) を $a-b$ と書く。
///
/// 空でない集合 $H\\subseteq G$ が $G$ の部分群であるとは、任意の $a, b\\in H$
/// に対して $ab\\in H$ かつ $a^{-1}\\in H$ が成り立つことを言う。
///
/// 群 $G$ の部分集合 $S$ は、集合 $\\angled{S}\\subseteq G$ を生成する (*generates*)。
/// ただし、$\\angled{S}$ は、$S$ に含まれる元とその逆元の有限回の積を含む。
/// $S = \\{g\_1, \\dots, g\_s\\}$ が有限のとき、単に $\\angled{g\_1, \\dots, g\_s}$ と書く。
///
...
///
/// - $0\\in R$ が存在して、$+$ に対して $R$ は可換群を成す。
/// - $\\times$ は結合律を満たし、単位元 $1\\in R$ を持つ。
/// - 分配律 (*distributivity*) を満たす。$a, b, c\\in R \\implies a(b+c)=(ab)+(ac), (b+c)a = (ba)+(ca)$。
///
/// さらに $\\times$ が可換律を満たすとき、その群は可換であると言う。
...
/// - $a\\in I, r \\in R \\implies a\\times r \\in I$
///
/// $R$ が可換のときは、単にイデアル (*ideal*) と言う。
/// 一般に、$a\_1, \\dots, a\_s$ によって生成されるイデアル (*ideal generated by ...*) を
/// $$
/// \\begin{aligned}
/// \\angled{a\_1, \\dots, a\_s}
/// &= a\_1 R + \\dots + a\_s R \\\\
/// &= \\{a\_1r\_1 + \\dots + a\_sr\_s \\mid r\_1, \\dots, r\_s\\in R\\}
/// \\end{aligned}
/// $$
/// と書き、$a\_1, \\dots, a\_s$ をそのイデアルの基底 (*basis*) と言う。
/// 特に、$\\angled{a} = aR = \\{ar\\mid r\\in R\\}$ を、$a\\in R$ によって生成される主イデアル (*principal ideal*) と言う。
///
/// $I\\subseteq R$ をイデアルとし、$r, s\\in R$ とする。$r$ と $s$ が $I$ を法として合同である
...
/// ## Definitions
///
/// 与えられた環 $R$ に対して、$R$ 上の一変数多項式 (*univariate polynomial*)
/// の環とは、$R$ の元からなるベクトル $a = (a\_0, a\_1, \\dots)$ のうち有限個の
/// $a\_i$ のみ非零であるものからなる集合 $S$ に、以下の加算と乗算が定義されたものを指す。
/// $$
/// \\begin{aligned}
/// (a\_0, a\_1, \\dots) + (b\_0, b\_1, \\dots) &= (a\_0+b\_0, a\_1+b\_1, \\dots), \\\\
/// (a\_0, a\_1, \\dots) \\times (b\_0, b\_1, \\dots) &= (c\_0, c\_1, \\dots).
/// \\end{aligned}
/// $$
/// ただし、$c\_n = \\sum\_{i=0}^n a\_ib\_{n-i}$ とする。
///
